...
const Verification:FC<Props> = ({setRoute}) => {
     ...
     const [invalidError, setInvalidError] = useState<boolean> (false);
       ...
       useEffect(()=> {
         ...
         if(error){
            const errorHasData = "data" in error
            if(errorHasData){
     ...
     const [verifiyNumber, setVerifyNumber] = useState<VerifiyNumber>({
        0:"",
        1:"",
        2:"",
        3:"",
     ...
     const handleInputChange = (index:number, value: string) => {
        ...
        const newVerifyNumber = {...verifiyNumber, [index]: value}
    ...
    <div>
     ...
     {Object.keys(verifiyNumber).map((key, index)=>(
        ...
        <h5 className="text-center pt-4 font-Poppins text-[14px] text-black dark:text-white">
            Go back to sign in? <span className="text-blue-500 pl-1 cursor-pointer" onClick={()=> setRoute("Login")}>
         Sign in
            </span>
       </h5>
    </div>
